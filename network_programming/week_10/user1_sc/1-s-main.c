...
int main( int argc, char * argv[])
{
	...
	/* inet socket values */
	int inet_serv_sock, inet_clnt_sock;
	struct sockaddr_in inet_serv_adr, inet_clnt_adr;
	socklen_t inet_clnt_adr_sz;
	...
	inet_serv_adr.sin_addr.s_addr = htonl(INADDR_ANY);
	inet_serv_adr.sin_port = htons(atoi(argv[1]));

	if(bind(inet_serv_sock, (struct sockaddr*)&inet_serv_adr, sizeof(inet_serv_adr)) == -1)
		error_handling("inet] bind() error");

	if(listen(inet_serv_sock, 5) == -1)
		error_handling("inet] listen() error");

	inet_clnt_adr_sz = sizeof(inet_clnt_adr);
	inet_clnt_sock = accept(inet_serv_sock, (struct sockaddr*)&inet_clnt_adr,&inet_clnt_adr_sz);
	if(inet_clnt_sock == -1)
		error_handling("inet] accpet() error");
	else
		printf("inet] Connected client\n");
	...
	/* end send message to another main server */

	/* close all socket */
	close(unix_clnt_sock);
	close(unix_serv_sock);
	close(inet_clnt_sock);
